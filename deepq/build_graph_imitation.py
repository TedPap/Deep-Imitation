...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        ment_obs_t_input = make_obs_ph("obs_t")
        # act_t_ph = tf.placeholder(tf.int32, [None], name="action")
        ment_obs_tp1_input = make_obs_ph("obs_tp1")
        ...
        # TED's compute the error
        aug_td_error = q_t_selected - tf.stop_gradient(q_t_selected_target)
        ...
        if grad_norm_clipping is not None:
            gradients = optimizer.compute_gradients(aug_weighted_error, var_list=q_func_vars)
            for i, (grad, var) in enumerate(gradients):
                if grad is not None:
                    gradients[i] = (tf.clip_by_norm(grad, grad_norm_clipping), var)
            aug_optimize_expr = optimizer.apply_gradients(gradients)
        else:
            aug_optimize_expr = optimizer.minimize(aug_weighted_error, var_list=q_func_vars)
        ...
        trainAugmented = U.function(
            ...
            outputs=aug_td_error,
            updates=[aug_optimize_expr]
        )
